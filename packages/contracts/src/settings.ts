...
import { DEFAULT_OMP_GIT_TEXT_GENERATION_MODEL, ProviderOptionSelections } from "./model.ts";
...
export const OmpSettings = makeProviderSettingsSchema(
  {
    ...
    agentDir: TrimmedString.pipe(
      ...
      Schema.annotateKey({
        ...
        description:
          "Optional OMP agent directory. Ashler-managed gateway overlays require this or ASHLER_OMP_AGENT_DIR.",
...
export const ServerSettings = Schema.Struct({
  // Ashler's primary UI contract is a live agent transcript. The websocket
  // layer still coalesces projection bursts on a bounded timer, so enabling
  // provider deltas here preserves upstream request-efficiency work without
  // delaying all visible output until the turn has completed.
  ...
  textGenerationModelSelection: ModelSelection.pipe(
    Schema.withDecodingDefault(
      Effect.succeed({
        ...
        model: DEFAULT_OMP_GIT_TEXT_GENERATION_MODEL,
      }),
    ),
  ),
