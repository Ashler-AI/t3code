...
it.layer(NodeServices.layer)("build-desktop-artifact", (it) => {
  ...
  it("switches desktop packaging product names to nightly for nightly builds", () => {
    assert.equal(resolveDesktopProductName("0.0.17"), "Ashler Code");
    assert.equal(resolveDesktopProductName("0.0.17-nightly.20260413.42"), "Ashler Code (Nightly)");
  });
  ...
  it("normalizes explicit macOS passkey RP domains and renders required entitlements", () => {
    ...
    assert.include(entitlements, "<string>ABC1234567.ai.ashler.code</string>");
  ...
  it.effect("adds passkey entitlements and both renderer protocols to signed macOS builds", () =>
    Effect.gen(function* () {
      ...
      assert.equal(config.appId, "ai.ashler.code");
      assert.equal(mac.entitlements, "/tmp/entitlements.mac.plist");
      assert.equal(mac.provisioningProfile, "/tmp/t3code.provisionprofile");
      assert.deepStrictEqual(mac.protocols, [
        { name: "Ashler Code", schemes: ["ashler-code", "ashler-code-dev"] },
      ]);
      assert.equal(mac.helperBundleId, "ai.ashler.code.helper");
    }).pipe(Effect.provide(ConfigProvider.layer(ConfigProvider.fromEnv({ env: {} })))),
  );

  it.effect("uses the Ashler product feed for packaged update metadata", () =>
    Effect.gen(function* () {
