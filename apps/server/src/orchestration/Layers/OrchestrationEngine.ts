...
const makeOrchestrationEngine = Effect.gen(function* () {
  ...
  const processEnvelope = (envelope: CommandEnvelope): Effect.Effect<void> => {
    ...
    return Effect.exit(
      Effect.gen(function* () {
        ...
        const attributedEventBases =
          envelope.metadata === undefined
            ? eventBases
            : eventBases.map((event) => ({
                ...event,
                metadata: { ...event.metadata, ...envelope.metadata },
              }));
        const committedCommand = yield* sql
          .withTransaction(
            Effect.gen(function* () {
              ...
              for (const nextEvent of attributedEventBases) {
  ...
  return {
    ...
    getAuthoritativeThreadById: (threadId) =>
      Effect.sync(() => Option.fromNullishOr(findThreadById(commandReadModel, threadId))),
