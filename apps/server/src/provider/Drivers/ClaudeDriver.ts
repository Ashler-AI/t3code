...
/** See the corresponding Codex interval for the recovery and serialization contract. */
export const CLAUDE_SNAPSHOT_REFRESH_INTERVAL = Duration.seconds(30);
  ...
  create: ({ instanceId, displayName, accentColor, environment, enabled, config }) =>
    Effect.gen(function* () {
      ...
      const snapshot = yield* makeManagedServerProvider<ProviderSnapshotSettings<ClaudeSettings>>({
        ...
        refreshInterval: CLAUDE_SNAPSHOT_REFRESH_INTERVAL,
      }).pipe(
        Effect.mapError(
          (cause) =>
